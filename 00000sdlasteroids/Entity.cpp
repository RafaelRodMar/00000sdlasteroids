...
void player::update()
{
	if (thrust)
	{
		m_velocity.m_x += cos((m_angle-90)*DEGTORAD)*0.2;
		m_velocity.m_y += sin((m_angle-90)*DEGTORAD)*0.2;
	}
	else
	{
		m_velocity.m_x *= 0.99;
	...
	int maxSpeed = 15;
	float speed = sqrt(m_velocity.m_x * m_velocity.m_x + m_velocity.m_y * m_velocity.m_y);
	if (speed > maxSpeed)
	{
		m_velocity.m_x *= maxSpeed / speed;
		m_velocity.m_y *= maxSpeed / speed;
	}
